...
Like the iframe fullscreen it is class-and-inline-style only — nothing moves in the DOM, which would reload an iframe. `popoutMap()` adds `.popout` (`position: fixed`), sets an inline `width` (`POPOUT_WIDTH`, or narrower if the block already was) and `left`/`top` at the block's on-screen position, so it reads as lifted rather than teleported, and drops a `.map-gap` spacer of the block's height after it, so the table does not jump; the spacer's *Vrati* and the button (now `[↙]`) both call `dockMap()`, which removes the class, the spacer and every inline property. Resizing is the browser's own `resize: horizontal` grip — the maps size their height from width (`aspect-ratio`, `.if1` padding), so width is the only dimension that needs controlling; `height: auto !important` discards the height the grip may also write, and `overflow: hidden`, which `resize` requires, does not clip a fullscreen map inside the widget because fixed descendants escape it. Dragging is a `pointerdown` on a non-fullscreen `.radartitle` inside a `.popout` (links and buttons excluded), with move/up listeners on `document`; `preventDefault()` there also suppresses the compatibility `mousedown`, so a drag by a slide title bar cannot register as a swipe on the slideshow around it, and `body.po-dragging` turns iframe pointer events off so the pointer is not swallowed mid-drag. Any `pointerdown` on a widget raises it (`popoutZ`), which is also what puts a widget's fullscreen above the other widgets — the fullscreen classes stack inside the widget's own context. `placePopout()` clamps into the viewport, on drag and on window resize alike.
