...
The arrangement is part of the view, so it travels with the map list rather than as a thing of its own: `snapLayout()` serialises the columns (per side the width and the panes as map ids with their height shares, fractions of the viewport rounded to four places, null when nothing is snapped) and it sits as `layout` next to the list in `mapPrefs`, in a saved preset (`{ id, name, maps, layout }`) and in the `?v=` payload. It is always a subset of the list it sits beside, and `sanitizeSnapLayout(layout, mapIds)` holds every copy read back to that — storage, links, saved entries: a pane must name a map in the list, once across both columns, up to the cap; shares are renormalised, the widths bounded to the viewport, a bad width drops its column, and a map dropped from the list leaves the layout. `applySnapLayout()` rebuilds the columns from a clean layout after a render (each pane popped out and attached as it stands through `attachSnapPane()`, then widths and shares set as stored); on load, `applyStoredSnapLayout()` does this on `DOMContentLoaded`, registered after the render so the maps are there — and after `main.js` has run in dev, since the snap code logs through `dlog()`. Desktop only, like the gestures: a phone carries the layout and does not show it.

The arrangement is written as it changes (`persistSnapLayout()` at the end of every snap, unsnap, handle drag and page toggle) — it is direct manipulation, not a form with an apply button — into `mapPrefs` next to whatever list is there. Under a shared view it goes into `sharedMapView` and back into the address bar instead, so the link stays re-copyable with the arrangement as it is now and a refresh keeps it, while the recipient's storage is still never written. The breakpoint docking everything runs with persistence paused (`snapPersistPaused`): that is the window changing, not the arrangement, and the stored layout is kept for a desktop window.

*Primijeni* decides which arrangement the new list gets (`layoutForPrefs()`): a saved preset's own — null included, a preset is the whole view as saved, so one saved with nothing snapped docks everything — and otherwise the one on screen, which a built-in or the custom list keeps as far as its maps allow; it goes into the preferences with the list and `applySnapLayout()` brings it back after the render drops the panes. Saving a preset (*Spremi*, *Ažuriraj*) stores the arrangement on screen held to the list in the picker (`selectedLayout()`), and `hasPendingEdits()` counts a changed arrangement as an edit, so a rearranged page offers *Ažuriraj* on the preset it was loaded from — only there, though: the origin dot on the chip marks a changed list, which is what the panel itself edits. Sharing carries it the same way: the panel's *Podijeli* with the list on screen, a saved preset's row with the preset's own (`presetSharePrefs()`), and a recipient who saves the shared view keeps it. The panel's one sign of all this is the `.ms-layout` line naming what is snapped (*Izdvojene karte: 2 lijevo, 1 desno*) with *Vrati sve* to dock it all.
