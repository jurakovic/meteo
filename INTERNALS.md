...
Floating widgets that touch — an edge of one on an edge of the other, overlapping along it (`rectsTouch()`, within `GROUP_TOUCH`), which is what the magnets leave — can be *grouped*, explicitly only: the title bar's `[+]` (`.grp-btn`, shown by `updateGroups()` only on a widget touching another or in a group; `[-]` on a member) joins the widget with what it touches, and with their groups, into one (`joinGroup()`), and `[-]` takes it out (`leaveGroup()`; a group left with one member is dissolved, and a group whose middle leaves stays a group though its ends no longer touch). A group lives in `block._group`, an id shared by its members, and shows as `.grouped` (the frame in the snap blue). It drags as one: `dragPopout()` moves the members by one offset (`groupStarts()`/`moveGroup()`), with the magnets and the viewport clamp seeing the group's bounding box (`groupBox()`) rather than the widget held, and a group never snaps into a column — only a single widget does. It raises as one too (`raisePopout()` brings the members up in their order), and the window-resize clamp moves a group by its box. Each member still resizes on its own, so a resize can open a gap the next drag's magnets close. Docking or snapping a member takes it out of its group. In the stored layout a group is a number shared by its members' floating entries (`group`, counted in order of appearance; `sanitizeSnapLayout()` drops one left with a single member), mapped back to fresh ids on restore.
