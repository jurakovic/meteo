...
})),e.addEventListener("error",(()=>{o++,dlog(`Error loading image: ${e.src}`),e.removeAttribute("src"),n()})),
e.complete&&e.dispatchEvent(new Event("load"))}))}function handleSwipe(e,t,o){const n=o-t;Math.abs(n)>50&&plusSlides(e,n>0?-1:1)}
function addSwipeEvents(){document.querySelectorAll(".slideshow").forEach((e=>{let t=0,o=0,n=0,r=0,s=!1
;const a=e.getAttribute("data-slideshow-id");e.querySelectorAll("img").forEach((e=>{e.addEventListener("dragstart",(e=>e.preventDefault()))
})),e.addEventListener("touchstart",(e=>{t=e.touches[0].clientX,o=e.touches[0].clientY,s=!1})),e.addEventListener("touchmove",(e=>{
n=e.touches[0].clientX,r=e.touches[0].clientY,Math.abs(n-t)>Math.abs(r-o)&&(s=!0)})),e.addEventListener("touchend",(()=>{if(s){
const e=n-t,s=r-o;Math.abs(e)>Math.abs(s)&&handleSwipe(a,t,n)}})),e.addEventListener("mousedown",(e=>{t=e.clientX,o=e.clientY,s=!0})),
e.addEventListener("mousemove",(e=>{s&&(n=e.clientX,r=e.clientY)})),e.addEventListener("mouseup",(e=>{if(s){n=e.clientX,r=e.clientY
;const i=n-t,l=r-o;Math.abs(i)>Math.abs(l)&&handleSwipe(a,t,n),s=!1}})),e.addEventListener("mouseleave",(()=>{s&&(s=!1)}))}))}
window.addEventListener("load",(function(){document.querySelectorAll("img.lazy").forEach((e=>{e.src=e.getAttribute("data-src"),
e.classList.remove("lazy")}))}));let previousWidth=0,zoomMap=new Map;function updateIframeSrc(){
window.innerWidth!==previousWidth&&(previousWidth=window.innerWidth,setIframeSrc("windy"),setIframeSrc("blitzortung"),
setIframeSrc("weatherAndRadar"),setIframeSrc("rainViewer"),setIframeSrc("ventussky"))}function setIframeSrc(e){
dlog(`Updating iframe src for ${e}`);const t=document.getElementById(e);if(t){
let o=zoomMap.get(e),n=o[0],r=o[1],s=t.getAttribute("data-src");window.innerWidth<800&&(s=s.replace(n,r)),t.src=s
...
e.removeAttribute("onclick");const t=e.cloneNode(!0);e.parentNode.replaceChild(t,e),t.addEventListener("click",(e=>{e.stopPropagation(),
...
;if(!t)return void dlog("essl img not found, skipping setEsslImgSrc");let o=GetLastInit();e>1&&o.setUTCHours(o.getUTCHours()-12*e)
...
if(e<4)dlog(`Image load failed, try attempt ${e} with dtg: ${r}`),t.removeEventListener("error",i),setEsslImgSrc(e+1);else{
