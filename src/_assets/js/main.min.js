...
;if(clearTimeout(o._hideTimer),t.style.width="0",o.style.removeProperty("display"),0===e.length)return void(o.style.display="none")
;const s=()=>{const s=n/e.length*100;t.style.width=s+"%",n===e.length&&(o._hideTimer=setTimeout(()=>{o.style.display="none"},500))}
;e.forEach(e=>{e.addEventListener("load",()=>{n++,s()}),e.addEventListener("error",()=>{n++,dlog(`Error loading image: ${e.src}`),
e.removeAttribute("src"),s()}),e.complete&&e.dispatchEvent(new Event("load"))})}function handleSwipe(e,t,o){const n=o-t;if(Math.abs(n)>50){
...
document.querySelectorAll(".links-bottom, .ms-presets").forEach(updateLinksScrollShadow)}function addLinksScrollShadows(){
